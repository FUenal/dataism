...
# library(GGally)
# 
# viz_1 <- ggpairs(trainData[,c(2:11,1)], aes(color=wrclmch, alpha=0.75), lower=list(continuous="smooth"))+ theme_bw()+
...
# ## Correlation Viz 2
# 
# viz_2 <- ggcorr(trainData[,c(2:11)], name = "corr", label = TRUE)+
...
# 
# viz_3 <- chart.Correlation(trainData[,c(2:10)],histogram=TRUE, col="grey10", pch=1, main="Climate Worry Mean")
...
viz_4 <- hchart(acc, 'line', hcaes(t, cnt)) %>%
...
# Plot
viz_5 <- hchart(acc, 'line', hcaes(k, cnt)) %>%
...
# Plot
viz_6 <- hchart(acc, 'line', hcaes(l, cnt)) %>%
...
# Plot
viz_7 <- hchart(acc, 'line', hcaes(p, cnt)) %>%
